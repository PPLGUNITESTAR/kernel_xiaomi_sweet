...
setup_environment() {
    ...
    # SUSFS Patch — standalone, applied independently of KSU variant
...
# NOTE: SUSFS is now handled separately via add_susfs().
#       KSU setup here ONLY handles KSU itself — no SUSFS logic inside.
add_ksu() {
...
# ---------------------------------------------------------------
# SUSFS integration — standalone, independent of KSU variant.
# Called AFTER add_ksu() so KSU headers/exports are already in tree.
# Only runs when KSU_SETUP_URI is non-empty (i.e. KSU is enabled).
# ---------------------------------------------------------------
add_susfs() {
    if [ -z "$KSU_SETUP_URI" ]; then
        echo "KSU not enabled, skipping SUSFS."
        return
    fi
    ...
    if [ $? -ne 0 ]; then
        echo "❌ ERROR: SUSFS patch failed to apply cleanly. Check for conflicts."
        exit 1
    fi

    echo "Appending SUSFS configs to defconfig..."
    cat >> "$MAIN_DEFCONFIG" << 'EOF'
CONFIG_KSU_SUSFS=y
...
main() {
    ...
    add_ln8k
    add_ksu
    add_susfs   # <- SUSFS applied after KSU, independently
    compile_kernel
}
